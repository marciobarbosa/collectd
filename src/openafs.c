 ...
 * Copyright (C) 2019       Marcio Barbosa
 *
...
openafs_destroy(void)
{
	...
	munmap(classp->counters, classp->n_counters * sizeof(unsigned int));
	close(classp->fd);
	free(classp->class);
...
static int
openafs_config_add(oconfig_item_t *a_ci, openafs_class_t *a_class)
{
    int status = 0;
...
_openafs_process_class(oconfig_item_t *a_ci, openafs_class_t *a_class)
{
	...
	if (status == 0) {
	    status = openafs_config_add(a_ci, a_class);
	}
    }
    return status;
}
...
_openafs_export_class(openafs_class_t *a_class)
{
    ...
    for (int i = 0; i < a_class->n_counters; i++) {
	snprintf(buffer, sizeof(buffer), "/%s/%s", a_class->class, a_class->labels[i]);
    }
}
